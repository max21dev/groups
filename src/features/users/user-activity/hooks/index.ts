...
export const useUserActivity = (pubkey: string) => {
  ...
  useEffect(() => {
    const subscribeToEvents = async () => {
      if (!ndk || !pubkey || selectedKinds.length === 0 || !isSubscriptionActive) return;
    ...
    if (ndk && pubkey && isSubscriptionActive && selectedKinds.length > 0) {
      subscribeToEvents();
    }
  ...
  }, [ndk, pubkey, isSubscriptionActive, selectedKinds]);
